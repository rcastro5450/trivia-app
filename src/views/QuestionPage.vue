...
<div v-if="question" class="flex h-full w-full flex-col items-center gap-8 p-10">
  <BaseTitle><MainScore></MainScore> &nbsp;
    <span class="font-bold" :class="notifcation === 'CORRECT' ? 'text-green-500' : 'text-red-500'">{{ notification }}</span>
  ...
  <div class="grid w-full flex-grow grid-cols-2 gap-8">
    ...
    class="bg-green-500 flex items-center justify-center text-4xl rounded-lg text-white py-10 px-5"></div>
    </div>
